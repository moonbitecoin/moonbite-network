...
MoonBite is a Scrypt proof-of-work coin (a Litecoin/Bitcoin Core fork). This guide
...
> Because MoonBite uses **Scrypt** (like Litecoin/Dogecoin), you cannot mine it
...
```bash
moonbited -regtest -daemon -server \
...
`YOUR_BIG_ADDRESS` with a MoonBite address (starts with `B`, or a bech32
...
`moonbited` RPC (port 9445) and point cpuminer at the bridge:

```bash
# 1) run moonbited with RPC enabled (see NODE_SETUP.md)
