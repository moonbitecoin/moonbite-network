# MoonBite Seed-Node Deployment Kit

This is what turns "MoonBite runs on my machine" into "a public network anyone on the
...
| `moonbited.service`  | systemd unit (auto-start, restart-on-failure, hardened) |
| `moonbite.conf`      | Seed-node config (public P2P, private RPC) |

## Quick deploy (per VPS)
```bash
# 1. Copy the release binaries + this deploy/ folder to the VPS, then:
sudo bash setup-seednode.sh ./moonbited ./moonbite-cli

# 2. Confirm it's running and reachable
systemctl status moonbited
moonbite-cli -conf=/etc/moonbite/moonbite.conf getnetworkinfo
```

The script installs binaries to `/usr/local/bin`, creates an unprivileged `moonbite`
...
2. On every node, uncomment the `addnode=` lines in `/etc/moonbite/moonbite.conf`
   and fill in the *other* seeds' IPs, then `systemctl restart moonbited`.
...
- `vSeeds.emplace_back("seed1.moonbite.org");`  (if you run a DNS seeder), and/or
